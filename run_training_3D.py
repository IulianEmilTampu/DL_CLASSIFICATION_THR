...
parser = argparse.ArgumentParser(description='Script that runs a cross-validation training for OCT 3D sparse volume classification. It uses the configuration file created using the configure_training.py file. Run the configuration first!')
...
for cv in folds:
    ...
    if config['model_configuration'] == 'LightOCT_3D':
        model = models_3D_tf.LightOCT_3D(num_classes=len(config['unique_labels']),
                    num_channels=1,
    ...
    elif config['model_configuration'] == 'M1_3D':
        model = models_3D_tf.M1_3D(num_classes=len(config['unique_labels']),
                    num_channels=1,
                    input_size=config['input_size'],
                    data_augmentation=False,
                    class_weights=config['class_weights'],
...
#     # test model
#     print(' - Testing fold...')
#     test_dataset = utilities.TFR_3D_sparse_dataset(config['test'],
#                     dataset_type = 'test',
#                     batch_size=config['batch_size'],
#                     buffer_size=1000,
#                     crop_size=config['input_size'])
