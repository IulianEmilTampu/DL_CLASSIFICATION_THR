"""
Implementation of different models used for OCT image classification. All models
share a custom training function that, for all models, saves the training and
...
class LightOCT(object):
    ...
    def __init__(
        ...
        inputs = Input(
            shape=[input_size[0], input_size[1], self.number_of_input_channels]
        )
...
class M2(object):
    ...
    def __init__(
        ...
        x = norm
        for i in n_filters:
            x = Conv2D(filters=i, kernel_size=self.kernel_size)(x)
...
class M4(object):
    ...
    def __init__(
        ...
        def Encoder_conv_block(inputs, n_filters):
            ...
            """
            y = Conv2D(
                filters=n_filters, kernel_size=(1, 1), padding="same", dilation_rate=1
            )(inputs)
            # perform conv with different kernel sizes
            conv3 = Conv2D(
                filters=n_filters, kernel_size=(3, 3), padding="same", dilation_rate=1
            )(inputs)
            conv5 = Conv2D(
                filters=n_filters, kernel_size=(3, 3), padding="same", dilation_rate=2
            )(inputs)
            conv7 = Conv2D(
                filters=n_filters, kernel_size=(3, 3), padding="same", dilation_rate=3
            )(inputs)
            ...
            if self.normalization == "BatchNorm":
                ...
                raise ValueError(
                    f"Not recognized normalization type. Expecting BatchNorm or GroupNorm but given {self.normalization}"
                )
...
class M6(object):
    def __init__(
        ...
        def conv_block(x, filters, kernel_size):
            ...
            x = Conv2D(filters=filters, kernel_size=self.kernel_size, padding="same")(x)
...
class VAE_original(object):
    ...
    def __init__(
        ...
        # build encoder with ResNet-like bloks
        # ¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤   conv block 1
        y = Conv2D(filters=32, kernel_size=self.kernel_size, padding="same")(
            augmented_norm
        )
        x = tfa.layers.GroupNormalization(groups=int(32 / 4))(y)
        x = tf.keras.layers.ReLU()(x)
        x = Conv2D(filters=32, kernel_size=self.kernel_size, padding="same")(x)
        # skip connection
        x = x + y
        # maxPool
        x = MaxPooling2D(pool_size=(2, 2), strides=2)(x)
        ...
        x = tfa.layers.GroupNormalization(groups=int(32 / 4))(x)
        x = tf.keras.layers.ReLU()(x)
        y = Conv2D(filters=128, kernel_size=self.kernel_size, padding="same")(x)
        ...
        def resize_convolution(x, final_shape, filters):
            upsample = tf.image.resize(
                images=x, size=final_shape, method=tf.image.ResizeMethod.BILINEAR
            )
            up_conv = Conv2D(
        ...
        # x = tf.keras.layers.LeakyReLU()(x)
        x = tf.keras.layers.Reshape((aus_dim[0], aus_dim[0], self.vae_latent_dim))(x)
        x = resize_convolution(
            x, final_shape=(aus_dim[0] * 2, aus_dim[0] * 2), filters=128
        )
        x = resize_convolution(
            x, final_shape=(aus_dim[0] * 4, aus_dim[0] * 4), filters=64
        )
        x = resize_convolution(
            x, final_shape=(aus_dim[0] * 8, aus_dim[0] * 8), filters=32
        )
        decoder_outputs = Conv2D(
            self.number_of_input_channels, 5, activation="tanh", padding="same"
        )(x)

        self.model = Model(
            inputs=inputs,
            outputs=[pred, decoder_outputs, augmented_norm, z_mean, z_log_var, z],
            name=model_name,
        )
...
class VAE1(object):
    ...
    def __init__(
        ...
        x = augmented_norm
        for i in n_filters:
            x = Conv2D(filters=i, kernel_size=kernel_size)(x)
            x = Conv2D(filters=i, kernel_size=kernel_size)(x)
            x = BatchNormalization()(x)
            x = LeakyReLU()(x)
            x = MaxPooling2D(pool_size=(2, 2), strides=2)(x)
...
class VAE2(object):
    ...
    def __init__(
        ...
        augmentor = tf.keras.Sequential(
            [layers.experimental.preprocessing.Normalization()], name="Normalization"
        )
        ...
        # build encoder with ResNet-like bloks
        # ¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤   conv block 1
        y = Conv2D(filters=32, kernel_size=self.kernel_size, padding="same")(x)
        x = tfa.layers.GroupNormalization(groups=int(32 / 4))(y)
        x = tf.keras.layers.ReLU()(x)
        x = Conv2D(filters=32, kernel_size=self.kernel_size, padding="same")(x)
        # skip connection
        ...
        x = tf.keras.layers.Conv2DTranspose(
            128, 5, activation="relu", strides=2, padding="same"
        )(x)
        x = tf.keras.layers.Conv2DTranspose(
            64, 5, activation="relu", strides=2, padding="same"
        )(x)
        x = tf.keras.layers.Conv2DTranspose(
            32, 5, activation="relu", strides=2, padding="same"
        )(x)

        decoder_outputs = tf.keras.layers.Conv2DTranspose(
            1, 5, activation="tanh", padding="same"
        )(x)
        self.model = Model(
            inputs=inputs,
            outputs=[pred, decoder_outputs, augmented_norm, z_mean, z_log_var, z],
...
class VAE4(object):
    def __init__(
        ...
        decoder_outputs = tf.keras.layers.Conv2DTranspose(
            self.number_of_input_channels, 3, activation="tanh", padding="same"
        )(x)
...
class VAE5(object):
    def __init__(
        ...
        def Decoder_conv_block(inputs, n_filters, final_shape):
            ...
            def resize_convolution(
                x, final_shape, filters, kernel_size=(3, 3), dilation_rate=1
            ):
                upsample = tf.image.resize(
                    images=x, size=final_shape, method=tf.image.ResizeMethod.BILINEAR
                )
            ...
            y = resize_convolution(
                inputs, final_shape, n_filters, kernel_size=(1, 1), dilation_rate=1
            )
            # perform resize_conv with different kernel sizes
            conv3 = resize_convolution(
                inputs, final_shape, n_filters, kernel_size=(3, 3), dilation_rate=1
            )
            conv5 = resize_convolution(
                inputs, final_shape, n_filters, kernel_size=(3, 3), dilation_rate=2
            )
            conv7 = resize_convolution(
                inputs, final_shape, n_filters, kernel_size=(3, 3), dilation_rate=3
            )
        ...
        # decode
        x = resize_convolution(
            x, n_filters=128, final_shape=(aus_dim[0] * 2, aus_dim[0] * 2)
        )
        x = resize_convolution(
            x, n_filters=64, final_shape=(aus_dim[0] * 4, aus_dim[0] * 4)
        )
        x = resize_convolution(
            x, n_filters=32, final_shape=(aus_dim[0] * 8, aus_dim[0] * 8)
        )
        # bring decoded image to original input number of channels
        decoder_outputs = Conv2D(
            filters=self.number_of_input_channels,
            kernel_size=(3, 3),
            padding="same",
            activation="tanh",
        )(x)
...
class ViT(object):
    def __init__(
        ...
        self.num_patches = (self.input_size[0] // self.patch_size) * (
