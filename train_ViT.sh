...
save_model_name="$model_configuration"_fold"$nFolds"_"$classification_type"_lr"$lr"_pts"$vit_patch_size"_prjd"$vit_projection_dim"_batch"$batchSize"
...
python3 -u run_training.py -cf $working_folder/trained_models/$save_model_name/config.json -db False -e 250 -p 250 |& tee -a $log_folder/$save_model_name.log
