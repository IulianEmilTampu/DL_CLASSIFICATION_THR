...
## 3D-LightOCT - 3d implementation of the 2D LightOCT model
class LightOCT_3D(object):
    def __init__(
        ...
        x = layers.Conv3D(filters=8, kernel_size=self.kernel_size, padding="same")(
            inputs
        )
...
class ViT_3D(object):
    def __init__(
        ...
        class PatchesFromSparseVolume(layers.Layer):
            ...
            def call(self, sparse_volume):
                ...
                patches_reshaped = tf.reshape(
                    patches,
                    [batch_size, n_patches, flattened_patch_dims, sequense_length],
                )
        ...
        class SparceVolumePatchEncoder(layers.Layer):
            def __init__(
                self, num_patches_per_img, img_sequence_length, projection_dim
            ):
            ...
            def call(self, patches):
                ...
                """
                in_img_positions = tf.range(
                    start=0, limit=self.num_patches_per_img, delta=1
                )
                in_sequence_positions = tf.range(
                    start=0, limit=self.img_sequence_length, delta=1
                )
                ...
                """
                encoded = self.projection(patches) + self.sequence_embedding(
                    in_sequence_positions
                )
                """
                embed in image position
                [batch, n_img_sequense, n_patches_per_img, project_dim] + [n_img_sequense, project_dim]
                """
                encoded = tf.transpose(encoded, perm=(0, 2, 1, 3)) + self.img_embedding(
                    in_img_positions
                )
                ...
                return tf.reshape(
                    ...
                    [
                        batch_size,
                        self.num_patches_per_img * self.img_sequence_length,
                        self.projection_dim,
                    ],
                )
        ...
        inputs = layers.Input(
            shape=(self.input_size[0], self.input_size[1], self.num_image_in_sequence)
        )
        ...
        # Encode patches.
        encoded_patches = SparceVolumePatchEncoder(
            self.num_patches, self.num_image_in_sequence, self.projection_dim
        )(patches)
...
class M4_3D(object):
    def __init__(
        ...
        def Encoder_conv_block(inputs, n_filters, kernel_size=3):
            ...
            x = layers.Conv3D(filters=n_filters, kernel_size=(1, 1, 1), padding="same")(
                x
            )
