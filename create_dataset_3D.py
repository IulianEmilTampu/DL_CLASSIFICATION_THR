...
introducing of interpolation that we do not want).

After the glass top part is removed, each 2D image in the 3D volumes is cropped
to match the final spatial size. The same image is also resampled to an isotropic
resolution using the specified resolution. All the images are collected back
together and a sparce sampling is performed sparce sampling is performed such that
15 slices are taken in a sparce way so that the overall depth is >= 0.3mm.
This means that depending on the resolution of the volume in the y direction,
one needs to find out which slides to take out.
...
belongs in the different classification types (3 for now - bynary, 4 classes and
5 classes). Samples can not be clastered based on the disease since the same
...
The isotropic sparce-volumes are then converted into TFRecords and saved in the
following way:
.../dataset_folder/
├── 2D_isotropic
...
This nomenclature adds from the previous the reference to the sparce volume in the
volume (TH02_0001_v1_c1_1_c2_2_c3_5_0003.extension), where the last 4 digits
(0003) identify the starting slice of the sparce collection.
...
IMPORTANT
Make sure to use a random number seed to generate the same partition
...
def get_indexes_sparse_volume(resolution, num_total_slices, depth=0.300, num_slices_per_volume=15):
    '''
    ...
    depth : float
        Depth that each sparced volume should have (marger in case resolution
    ...
    if num_total_slices * resolution < depth:
        ...
        space_between_slices = int(np.round(num_slides_to_cover_depth/num_slices_per_volume))
        master_sequence = list(range(0,num_slides_to_cover_depth, space_between_slices))
...
# slices = get_indexes_sparse_volume(0.0025, 285, 0.3, 15)
...
# parser.add_argument('-sd','--sparse_depth', required=True, help='Depth of the sparse volume in the y direction (stack of b-scans) in mm.')
# parser.add_argument('-sns','--sparse_num_slices', required=True, help='Number of b-scans in each sparse 3D volume covering sparse_depth.')
...
if not os.path.isfile(dataset_specs):
    raise TypeError('Invalid dataset specification file. Given {} but the file does not exist'.format(dataset_specs))
...
for counter, f in enumerate(files):
    ...
    slices = get_indexes_sparse_volume(resolution=anisotropic_res[-1],
    ...
    if slices:
        ...
        for idx, sparse_slices in enumerate(slices):
            ...
            # save .nii
            nib.save(nib.Nifti1Image(sparse_volume, volume_template.affine, volume_template.header),
...
json_dict['name'] = "Sparse3D_OCT_Thyroid_Classification"
json_dict['description'] = "Sparse 3D dataset of OCT b-scan images (anisotropic and isotropic) of normal and diseased thyroid tissue"
