...
files = [
    ...
    for x in glob.glob(os.path.join(dataset_folder, "*"))
]
...
for c in classes:
    ...
    for v in per_class_random_volumes[c]:
        ...
        index_of_selected_files.extend(
            [i for i, f in enumerate(files) if v["file_name"] in f]
        )
...
# the other classes
per_classification_test_files["c1"] = random.sample(
    per_class_random_files["normal"], n_images_per_class
)
n_other_classes = int(np.ceil(n_images_per_class / 5))

per_classification_test_files["c1"].extend(
    random.sample(per_class_random_files["goiter"], n_other_classes)
)
per_classification_test_files["c1"].extend(
    random.sample(per_class_random_files["adenoma"], n_other_classes)
)
per_classification_test_files["c1"].extend(
    random.sample(per_class_random_files["graves"], n_other_classes)
)
per_classification_test_files["c1"].extend(
    random.sample(per_class_random_files["hashimoto"], n_other_classes)
)
per_classification_test_files["c1"].extend(
    random.sample(per_class_random_files["cancer_shrunk"], n_other_classes // 2)
)
per_classification_test_files["c1"].extend(
    random.sample(per_class_random_files["cancer_depleted"], n_other_classes // 2)
)
...
# that are depleted)
per_classification_test_files["c2"] = random.sample(
    per_class_random_files["normal"], n_images_per_class
)
per_classification_test_files["c2"].extend(
    random.sample(per_class_random_files["goiter"], n_images_per_class)
)
per_classification_test_files["c2"].extend(
    random.sample(per_class_random_files["adenoma"], n_images_per_class // 4)
)
per_classification_test_files["c2"].extend(
    random.sample(per_class_random_files["graves"], n_images_per_class // 4)
)
per_classification_test_files["c2"].extend(
    random.sample(per_class_random_files["hashimoto"], n_images_per_class // 4)
)
per_classification_test_files["c2"].extend(
    random.sample(per_class_random_files["cancer_shrunk"], n_images_per_class // 4)
)
per_classification_test_files["c2"].extend(
    random.sample(per_class_random_files["cancer_depleted"], n_images_per_class)
)
...
per_classification_test_files["c3"] = random.sample(
    per_class_random_files["normal"], n_images_per_class
)
per_classification_test_files["c3"].extend(
    random.sample(per_class_random_files["goiter"], n_images_per_class)
)
per_classification_test_files["c3"].extend(
    random.sample(per_class_random_files["adenoma"], n_images_per_class)
)
per_classification_test_files["c3"].extend(
    random.sample(per_class_random_files["graves"], n_images_per_class)
)
per_classification_test_files["c3"].extend(
    random.sample(per_class_random_files["hashimoto"], n_images_per_class)
)
per_classification_test_files["c3"].extend(
    random.sample(per_class_random_files["cancer_shrunk"], n_images_per_class // 2)
)
per_classification_test_files["c3"].extend(
    random.sample(per_class_random_files["cancer_depleted"], n_images_per_class // 2)
)
...
remaining_test_files = [
    f for i, f in enumerate(files) if i not in index_of_selected_files
]
...
with open(
    os.path.join(os.path.dirname(dataset_folder), "train_test_split.json"), "w"
) as fp:
